...
static int	check_elements(char *line, t_validator *v)
{
		...
		&& v->total_positions <= 1)
	{
		...
		{
			result = FALSE;
			display_message(ERROR_INVALID_SYMBOL, 44);
		}
		line++;
	}
	return (result);
...
static int	check_counter(char *line, t_validator *v, int type)
{
	...
	if ((v->total_exits == 0 || v->total_positions == 0)
		&& type == LAST_LINE)
	{
		display_message(ERROR_FEW_ELEM, 39);
		result = FALSE;
	}
	else if ((v->total_exits > MAX_EXITS || v->total_positions > MAX_POSITIONS)
		&& type == MIDDLE_LINE)
	{
		display_message(ERROR_MORE_ELEM, 45);
		result = FALSE;
	}
	return (result);
}
		...
		size_t len_first_line)
{
	...
	if ((type == MIDDLE_LINE || type == LAST_LINE))
	{
		...
		{
			display_message(ERROR_INVALID_SIZE, 28);
			result = FALSE;
		}
		...
		{
			display_message(ERROR_INVALID_BORDER, 32);
			result = FALSE;
		}
	}
